...
/**
 * Example that visualizes an arbitrary number of CellSpaces.
 * Each CellSpace is colored in sequence and the centroids are
 * connected in a ring of transitions.
 */
public class MultiCellSpacesExample extends SimpleApplication {

    private ChaseCamera chaseCam;

    public static void main(String[] args) {
        MultiCellSpacesExample app = new MultiCellSpacesExample();
        app.start();
    }

    @Override
    public void simpleInitApp() {
        List<List<Polygon>> cellSpaces = createCellSpaces();

        // gather all polygons for bounding calculations
        List<Polygon> allPolygons = new ArrayList<>();
        for (List<Polygon> cs : cellSpaces) {
            allPolygons.addAll(cs);
        }
        ...
        for (int i = 0; i < cellSpaces.size(); i++) {
            ColorRGBA color = colors[i % colors.length];
            scene.attachChild(CellSpaceGeometryBuilder.buildCellSpaces(
                    cellSpaces.get(i), assetManager, color, 0.5f));
        }
    ...
    private List<List<Polygon>> createCellSpaces() {
        List<List<Polygon>> spaces = new ArrayList<>();
        spaces.add(createCellSpace1());
        spaces.add(createCellSpace2());
        return spaces;
    }

    private List<StatePoint> createStates(List<List<Polygon>> cells) {
        List<StatePoint> states = new ArrayList<>();
        for (List<Polygon> cell : cells) {
            states.add(computeCentroid(cell));
        }
        return states;
    }
    ...
    private List<LineString> createTransitions(List<StatePoint> states) {
        ...
        for (int i = 0; i < states.size(); i++) {
            ...
            lines.addAll(createTransition(a, b));
        }
        return lines;
    }
